...
- **最终实测**：最终字典 2359 条（9 个分批字典合并 2362 条、剔除 3 个节点外部比较串）；439 个 Java 类、2444 处字面量替换；成品镜像 477 个资源含中文、共 20481 个中文字符；残留英文均为有意保留项（SQL/协议日志/专名/外部错误原文）
...
- 打包 `Sparrow-2.5.4-中文便携版.zip`（97,574,955 字节，SHA256 `49765018990A2FD17655FE76A134C1AEB1B900D8554DC7A25BEA072C28F8FF1F`）；便携版只含开箱即用的中文程序 + 最终文档 + 轻量补丁工具链（9 脚本 / 10 字典），不重复捆绑近 90MB 的镜像（原始/成品镜像作为 Release 单独资产）；中文文件名统一为标准 UTF-8，跨平台解压不乱码

### 目录体积治理
- 程序目录一度约 2.0 GB；删除重复解包副本、jlink 失败产物、重复 JDK、源码包、临时镜像/截图/中间清单等过程文件
- 最终根目录约 384 MB（便携版 zip 约 93 MB、中文补丁约 168 MB、运行时 runtime 约 122 MB）；便携版经去重重打由 249.6 MB 降至 93.1 MB；构建工具链不再需要 JDK
...
- 一键构建/应用/还原脚本、翻译字典（FXML + Java 9 个分批明文字典，构建时自动合并为 2359 条并剔除外部比较串黑名单）
