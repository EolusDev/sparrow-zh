...
2. 运行 `build.py` 对新镜像重新构建。绝大部分界面文字沿用现有 2330 条字典
3. 用 `scan_remaining.py` 扫描新版里尚未覆盖的英文字面量
4. 把新增可见文案补进 `翻译字典\java_trans_all.json`（或 fxml_trans.json），重跑 `build.py` 即可
...
- **完整汉化**：31 个界面布局文件（FXML）+ 439 个 Java 类、2442 处界面文字中文化，成品镜像共 477 个资源含中文、20473 个中文字符
- **翻译字典 2330 条**：人工分批审定，覆盖主界面、设置、交易、UTXO、多签、24 款硬件钱包导入向导、Payjoin、消息签名、私钥清扫、助记词等全部界面
- **纯 Python 工具链**：自行实现 jimage 解包 / 常量池改写 / 镜像重建 / 校验，只用标准库，克隆后无需 JDK 即可复现
...
│   └── patch/
│       ├── 翻译字典/            # fxml_trans.json + java_trans_all.json.gz 等（可扩展）
│       └── 脚本/
...
| Java 界面字面量 | 439 个类、2442 处替换（字典 2330 条） |
| 成品镜像 | 477 个资源含中文，共 20473 个中文字符；57359 个资源完美哈希 0 失配 |
